...
router.get("/feed", auth.verifyToken, async function(req, res) {
  try {
    ...
    await Article.find({ author: { $in: currentUser.following } })
      .sort({ createdAt: -1 })
      .limit(20)
      .exec((err, feed) => {
...
//delete comments
router.delete("/:slug/comments/:id", auth.verifyToken, async function(
  req,
  res
) {
  try {
...
router.post("/:slug/favorite", auth.verifyToken, async function(req, res) {
  try {
    var article = await Article.findOne({
      slug: req.params.slug
    }).populate("author", ["bio", "username", "image"]);
